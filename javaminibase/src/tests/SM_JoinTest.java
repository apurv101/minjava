...
class JoinsDriver implements GlobalConst {
  ...
  public JoinsDriver() {
    ...
    // Build Sailors, Boats, Reserves data
    sailors  = new Vector();
    boats    = new Vector();
    reserves = new Vector();
    ...
    for (int i = 0; i < sailors.size(); i++) {
      try {
        Sailor s = (Sailor)sailors.elementAt(i);
        t.setIntFld(1, s.sid);
        t.setStrFld(2, s.sname);
        t.setIntFld(3, s.rating);
        t.setFloFld(4, (float)s.age);
      }
      catch (Exception e) {
    ...
    for (int i = 0; i < boats.size(); i++) {
      try {
        Boats b = (Boats)boats.elementAt(i);
        t.setIntFld(1, b.bid);
        t.setStrFld(2, b.bname);
        t.setStrFld(3, b.color);
      }
      catch (Exception e) {
    ...
    for (int i = 0; i < reserves.size(); i++) {
      try {
        Reserves rsv = (Reserves)reserves.elementAt(i);
        t.setIntFld(1, rsv.sid);
        t.setIntFld(2, rsv.bid);
        t.setStrFld(3, rsv.date);
      }
      catch (Exception e) {
        System.err.println("*** error in Tuple.setStrFld() ***");
        status = FAIL;
  ...
  // -----------------------------------------------------
  // Helpers for building conditional expressions
  // -----------------------------------------------------
  ...
  // -----------------------------------------------------
  public void Query1() {
    PCounter.initialize(); // Reset counters
    ...
    if (_status != OK) {
      System.err.println("*** Error in get_next() for Query1");
      Runtime.getRuntime().exit(1);
    }
    ...
    System.out.println("\n");
    if (_status != OK) {
      System.err.println("*** Error in closing for Query1");
      Runtime.getRuntime().exit(1);
    }

    // Print counters for Query1
    System.out.println("Query1 - Read I/Os  : " + PCounter.rcounter);
    System.out.println("Query1 - Write I/Os : " + PCounter.wcounter);
  }

  // -----------------------------------------------------
  // Query2 (currently empty, but let's still show counters)
  // -----------------------------------------------------
  public void Query2() {
    PCounter.initialize();
    System.out.println("********************** Query2 starting *********************");
    // No test logic here in the snippet, but we still demonstrate counters.

    // ... If you add real logic, place it here ...
  ...
  public void Query5() {
    ...
    System.out.println("\n");
    if (status != OK) {
      System.err.println("*** Error closing SortMerge for Query5");
      Runtime.getRuntime().exit(1);
    }

    // Print counters for Query5
    System.out.println("Query5 - Read I/Os  : " + PCounter.rcounter);
    System.out.println("Query5 - Write I/Os : " + PCounter.wcounter);
  }

  // -----------------------------------------------------
  // Query6 (empty, but we still show counters)
  // -----------------------------------------------------
  public void Query6() {
    PCounter.initialize();
    System.out.println("********************** Query6 starting *********************");
    // If you add actual query logic, do it here
