...
public class Sort extends Iterator implements GlobalConst
{
  ...
  private int _countFetched = 0;
    private List<Tuple> sortedTuples;  // Store precomputed sorted tuples
 ...
 */
  ...
  public Sort(
      ...
      _k = k;
      _isVectorSort = (in[sort_fld - 1].attrType == AttrType.attrVector100D);
      sortedTuples = new ArrayList<>();
      Tuple t;

      // Fetch and store  in list
      while ((t = am.get_next()) != null) {
          sortedTuples.add(new Tuple(t)); // Copy tuple to prevent overwrites
      }

      // Sorting  for 100D vector field
      if (_isVectorSort) {
          sortedTuples.sort((t1, t2) -> {
              try {
                  Vector100Dtype v1 = t1.get100DVectFld(sort_fld);
                  Vector100Dtype v2 = t2.get100DVectFld(sort_fld);
                  int d1 = computeDistance(v1, _targetVector);
                  int d2 = computeDistance(v2, _targetVector);
          ...
          if (_k > 0 && sortedTuples.size() > _k) {
              sortedTuples = sortedTuples.subList(0, _k);
          }
      }
      sortedIndex = 0; // Reset index for iteration
  }
	   ...
	   Exception
  {
      ...
      if (_isVectorSort) {
          ...
          if (sortedIndex < sortedTuples.size()) {
              return sortedTuples.get(sortedIndex++);
          }
          return null;
      }
