...
class DBDriver extends TestDriver implements GlobalConst {
  ...
  protected boolean test1() {
    ...
    if (status == OK) {
      ...
      for (int i = 0; i < 20 && status == OK; ++i) {
        ...
        String writeStr = "A" + i;
        byte[] data = new byte[2 * writeStr.length()]; // enough space
        try {
  ...
  protected boolean test3() {
    ...
    if (status == OK) {
      ...
      try {
        ...
        if (pgid == null) {
          // No exception should be thrown in this case, but if pgid == null, that indicates
          // the entry is truly gone, which might or might not be what's expected.
          status = FAIL;
    ...
    if (status == OK) {
      ...
      // Create a byte array that is big enough to fail
      char[] data = new char[MAX_NAME + 5];
      for (int i = 0; i < MAX_NAME + 5; i++) {
        data[i] = 'x';
      }

      String name = new String(data); // big name
      try {
    ...
    if (status == OK) {
      ...
      try {
        SystemDefs.JavabaseDB.allocate_page(pgid, -10);
      } catch (java.io.IOException e) {
        System.err.println(" IOerror: " + e);
    ...
    if (status == OK) {
      ...
      try {
        SystemDefs.JavabaseDB.deallocate_page(pgid, -10);
      } catch (java.io.IOException e) {
        System.err.println(" IOerror: " + e);
      } catch (InvalidRunSizeException e) {
  ...
  protected boolean test4() {
    ...
    // We create a new database that's big enough to require 2 pages for space map.
    ...
    if (status == OK) {
      System.out.print("  - Allocate all pages remaining after DB overhead is accounted for\n");
      try {
        ...
        status = FAIL;
        e.printStackTrace();
        System.err.print("*** Too little space available: could not allocate " + (dbsize - 3) + " pages\n");
      }
    ...
    if (status == OK) {
      ...
      try {
        SystemDefs.JavabaseDB.deallocate_page(pg11, 7); // pages 11-17
      } catch (java.io.IOException e) {
        System.err.println(" IOerror: " + e);
        status = FAIL;
        System.err.println("*** Error deallocating pages\n");
      ...
      if (status == OK) {
        try {
          SystemDefs.JavabaseDB.deallocate_page(pg18, 11); // pages 18-28
        } catch (java.io.IOException e) {
          System.err.println(" IOerror: " + e);
          status = FAIL;
          System.err.print("*** Error deallocating pages\n");
    ...
    if (status == OK) {
      ...
      try {
        ...
        System.err.println(" IOerror: " + e);
        status = FAIL;
        System.err.print("*** Error allocating pages\n");
      } catch (Exception e) {
        status = FAIL;
        System.err.print("*** Could not allocate pages\n");
        e.printStackTrace();
    ...
    if (status == OK) {
      System.out.print("  - Add enough file entries that the directory " +
        "must surpass one page\n");
      // Over-count, but uses only public info.
      int count = MAX_SPACE / MAX_NAME + 1; // e.g. 21

      for (int i = 0; i < count && status == OK; ++i) {
        String name = "file" + i;
        // Set every file's first page to be page 0
        try {
          SystemDefs.JavabaseDB.add_file_entry(name, new PageId(0));
        } catch (Exception e) {
          status = FAIL;
    ...
    if (status == OK) {
      System.out.print("  - Make sure the directory has taken up an extra page.\n" +
    ...
    if (status == OK) {
      ...
      try {
        SystemDefs.JavabaseDB.allocate_page(pgid); // 1 page
      } catch (java.io.IOException e) {
        System.err.println(" IOerror: " + e);
      } catch (OutOfSpaceException e) {
        status = FAIL;
    ...
    if (status == OK) {
      System.out.print("  - Free the last two pages: boundary condition in the space map.\n");
      try {
        ...
        status = FAIL;
        System.err.print("*** Deallocate last two pages did not work.\n");
        e.printStackTrace();
      }
  ...
  protected boolean test5() {
    ...
    // There's no real logic here, so let's just say it passes.
    boolean status = OK;
  ...
  protected boolean test6() {
    ...
    // Same as test5: no logic, just pass
    boolean status = OK;
