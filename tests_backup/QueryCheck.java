...
/*
  struct Group{
    int len;                 // The num of tuples in the group.
...
public class QueryCheck {
  ...
  public QueryCheck(int q_index) {
    ...
    // ------------------------------------------
    // 1) Reset the counters at the start
    // ------------------------------------------
    PCounter.initialize();
    ...
    // Fill the "correct answer" vectors
    ...
    for (int i = 1; i <= fldnum; i++) {
      switch ((type[i-1]).attrType) {
        ...
        case AttrType.attrString:
          ...
          break;
        default:
          // error (Don't know what to do with attrSymbol, attrNull, etc.)
          break;
      }
    }
  }
  ...
  public void Check(Tuple t) {
    ...
    if (mygroup[curGroup].order.order == Order.SORT) {
      // check if it matches the "next expected tuple"
      try {
  ...
  void MisMatch(Tuple t) {
    ...
    if (tempGroup == -1) {   // t not in answer tuples
      ...
      // it should match in the current group but wasn't "next"
      if (mygroup[curGroup].order.order == Order.UNSORT) {
        // This shouldn't happen unless we made a logic error
        System.out.print("*****Tuple in current group, but not found in normal check.\n\n");
        return;
      }
      else {
      ...
      if (mygroup[curGroup].count != 0) {
        // add the remaining unmarked tuples in curGroup to missing
      ...
      if (grouporder.order == Order.SORT) {
        ...
        if (tempGroup != curGroup) {
          ...
          G_O_flag = 1;
          // change the grouporder to UNSORT
  ...
  void MarkTuple(int groupNum, int tupleNum) {
    ...
    // check to see if current group is done
  ...
  public void report(int querynum) {
    ...
    if (total == tuplenum && missing == null && extra == null
        && G_O_flag == 0 && t_order_error == 0) {
      System.out.print("\nQuery" + querynum + " completed successfully!\n");
      System.out.print("******************* Query" + querynum 
                       + " finished!!! *****************\n\n");
    }

    // ------------------------------------------
    // 2) Print the counters at the end
    // ------------------------------------------
    System.out.println("Query" + querynum + " - Read I/Os  : " + PCounter.rcounter);
    System.out.println("Query" + querynum + " - Write I/Os : " + PCounter.wcounter);
  }
}
