...
class SORTDriver extends TestDriver implements GlobalConst {
  ...
  protected boolean test1()
  {
    ...
    // Sort "test1.in" on field 1, ascending
    Sort sort = null;
    try {
  ...
  protected boolean test2()
  {
    ...
    // Sort "test2.in" on field 1, descending
    Sort sort = null;
    try {
  ...
  protected boolean test3()
  {
    ...
    while (t != null) {
      ...
      if (iout < ival) {
        ...
        System.err.println("Test3 -- OOPS! test3.out not sorted (int asc)");
        status = FAIL;
        break; 
      }
      count++;
    ...
    if (count < LARGE) {
      System.err.println("Test3 -- OOPS! too few records (int asc)");
      status = FAIL;
    }
    else if (flag && status) {
    ...
    // Now sort "test3.in" on the float attribute (field 4) Descending
    ...
    while (t != null) {
      if (count >= LARGE) {
        System.err.println("Test3 -- OOPS! too many records (float desc)");
        status = FAIL;
        flag = false; 
        break;
      }
      ...
      if (fout > fval) {
        ...
        System.err.println("Test3 -- OOPS! test3.out not sorted (float desc)");
        status = FAIL;
        break; 
      }
      count++;
    ...
    if (count < LARGE) {
      System.err.println("Test3 -- OOPS! too few records (float desc)");
      status = FAIL;
    }
    else if (flag && status) {
  ...
  protected boolean test4()
  {
    ...
    // create an iterator by open two file scans
    FldSpec[] projlist = new FldSpec[2];
    RelSpec rel = new RelSpec(RelSpec.outer); 
    projlist[0] = new FldSpec(rel, 1);
    projlist[1] = new FldSpec(rel, 2);
    ...
    // Sort each input file differently
    Sort sort1 = null;
    Sort sort2 = null;
    try {
    ...
    while (t1 != null) {
      ...
      if (outval.compareTo(data2[count]) != 0) {
        System.err.println("outval = " + outval + "\tdata2[count] = " + data2[count]);
        System.err.println("Test4 -- OOPS! test4.out not sorted (asc side)");
        status = FAIL;
      }
      count++;
  ...
  {
    // Even if there's no real logic, we can still show counters:
    PCounter.initialize(); 
    // (some hypothetical test, returning true)
    System.out.println("Test5 - Nothing done, so counters are minimal.");
  ...
  protected boolean test6()
  {
    PCounter.initialize(); 
    // (some hypothetical test, returning true)
    System.out.println("Test6 - Nothing done, so counters are minimal.");
